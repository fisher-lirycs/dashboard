...
const Clock: React.FC<ClockProps> = ({ width = "100%", height = "100%", borderColor = "#333333", time }) => {
    ...
    const drawClock = useCallback((time: Date, frameWidth: number, canvas: HTMLCanvasElement, ctx: CanvasRenderingContext2D) => {
        ...
        for (let j = 0; j < 60; j++) {
            ...
            ctx.arc(x, y, 2 * ratio, 2 * Math.PI, 0, false);
            ctx.fill();
            ctx.closePath();
        }
        ...
        ctx.beginPath();
        ctx.fillStyle = "#fff";
        ctx.arc(0, 0, 3 * ratio, 2 * Math.PI, 0, false);
        ctx.fill();
    ...
    useEffect(() => {
        ...
        if (canvasRef.current) {
            ...
            if (context) {
                ...
                const render = () => {
                    drawClock(time, frameWidth, canvas, context)
                }
                render()
            }
        }
