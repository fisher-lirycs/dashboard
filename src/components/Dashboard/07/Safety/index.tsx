...
const Safety: React.FC<SafetyProps> = ({
  ...
  const [safetyDetail, setSafeDetail] = useState(
    localStorage.getItem("safetyDetail") ||
      "私はルールを正しく守り、\r\n自分の身は自分で守ります"
  );
  ...
  return (
    <SafetyContainer>
      ...
      {editStatus ? (
        ...
        <TextContent fontSize={fontSize}>{safetyDetail}</TextContent>
      )}
    </SafetyContainer>
  );
};
